...
# Each field creator emits a single __preview_* marker call, expanded to the classification SQL in the
# resolver. These assert the field emits the right marker over its user-agent expression; the expansion
# structure (multiMatchAnyIndex, labels, defaults) is covered in test_traffic_type_functions.py.
FIELD_MARKERS = [
    (create_is_bot_field, "$virt_is_bot", "__preview_isBot"),
    (create_traffic_type_field, "$virt_traffic_type", "__preview_getTrafficType"),
    (create_traffic_category_field, "$virt_traffic_category", "__preview_getTrafficCategory"),
    (create_bot_name_field, "$virt_bot_name", "__preview_getBotName"),
    (create_bot_operator_field, "$virt_bot_operator", "__preview_getBotOperator"),
]
